...
| 句边界处双重起音（火星语一遍+正常重念一遍） | `<d>` 分段时间线与参考音频实际句子边界错位，模型被两个信号各触发一次起音 | VAD 实测音频句界，切点对齐到句间静音内（见 §7） |
...
| 现象 | 原因 | 修复 |
|------|------|------|
| 句末随机吞字（每次吞哪个字不稳定） | `<d>` 字数多于参考音频对应句的节奏容量，模型在旧节奏里塞新词，句末超载随机丢音节 | 缩字：`<d>` 字数 ≤ 音频对应句字数（少 1 字留余量反而稳）；治本则重录音频使内容一致 |
...
## 7. reference 模式时间线工程（切点对齐 + 吞字防治）

来源：沉浸开箱画外音版完整排查链（火星语→双重起音→吞字，五轮迭代根治）。
...
- [ ] 镜头切点已按 VAD 实测句界对齐（换过音频必重测）
- [ ] `<d>` 各句字数 ≤ 参考音频对应句字数（句末留余量）
- [ ] 句中后段无轻读虚词（都/也/就/会/了）
...
| 沉浸开箱画外音版 | 5 镜头：人物 → 俯拍开箱 → 滴管特写 → 面霜特写 → 人物捧盒 | 开箱动作 + 质地特写（一镜到底变体可行） | 全部画外音（首尾露脸闭唇），切点 VAD 对齐 3.3/6.8/10.7/13.8s |
